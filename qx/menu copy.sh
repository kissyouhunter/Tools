					...
					case "$SUBMENU1" in
						1 )
							...
							function docker_menu() {
								DOCKER_MENU=$(whiptail --title "一键脚本 作者：kissyouhunte" --checklist \
								"请选择" 15 60 2 \
								"DOCKER" "" ON \
								"DOCKER-COMPOSE" "" ON 3>&1 1>&2 2>&3)
								
								exitstatus=$?
								if [ $exitstatus = 0 ]; then
    								echo "Your favorite distros are:" $DOCKER_MENU
								else
    								main
								fi
							}
							...
							if [ "$lsb_dist" == "openwrt" ]; then
								...
								sleep 1
        						bash <(curl -s -S -L https://raw.githubusercontent.com/kissyouhunter/Tools/main/docker-and-docker_compose.sh) | whiptail --gauge "程序安装中，请稍等" 6 60 0
